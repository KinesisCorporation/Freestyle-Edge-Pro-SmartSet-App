...
begin
  ...
  begin
    ...
    begin
      ...
      result := FS_FILENAME + FormSaveAs.cboLayout.Text + '.txt';
    end
    else
    begin
      backupFile := true;
...
begin
  ...
  begin
    ShowDialog('Save As', 'You must select a layout position 1 to 9', mtError, [mbOK], DEFAULT_DIAG_HEIGHT,
          KINESIS_DARK_GRAY_FS, clWhite);
    cboLayout.SetFocus;
  end
  else if (rgCustom.Checked) and (Trim(eCustLayout.Text) = '') then
  begin
    ShowDialog('Save As', 'You must enter a backup file name', mtError, [mbOK], DEFAULT_DIAG_HEIGHT,
          KINESIS_DARK_GRAY_FS, clWhite);
