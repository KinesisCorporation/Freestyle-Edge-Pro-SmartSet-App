...
begin
  ...
  begin
    ...
    begin
      result := FS_FILENAME + FormLoad.cboLayout.Text + '.txt';
    end
    else
    begin
...
begin
  ...
  begin
    ...
    if (LowerCase(Copy(fileName, 1, Length(FS_FILENAME))) <> LowerCase(FS_FILENAME)) and (Copy(fileName, 1, 1) <> '.') then
...
begin
  ...
  begin
    ShowDialog('Load', 'You must select a layout position 1 to 9', mtError, [mbOK], DEFAULT_DIAG_HEIGHT,
          KINESIS_DARK_GRAY_FS, clWhite);
    cboLayout.SetFocus;
  end
  else if (rgCustom.Checked) and (Trim(cboBackup.Text) = '') then
  begin
    ShowDialog('Load', 'You must select a backup layout', mtError, [mbOK], DEFAULT_DIAG_HEIGHT,
          KINESIS_DARK_GRAY_FS, clWhite);
    cboBackup.SetFocus;
  end
  else
